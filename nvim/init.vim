...
if not ok then
  ...
  local claude_model = 'claude-sonnet-5'
  local claude_bin = '/opt/homebrew/bin/claude'
  local setup_ok, setup_err = pcall(claudecode.setup, {
    terminal_cmd = claude_bin .. ' -c --model ' .. claude_model .. ' || ' .. claude_bin .. ' --model ' .. claude_model,
...
EOF
lua << EOF
-- nvim-treesitter is on the `main` branch (rewrite): no more
-- `nvim-treesitter.configs` / `ensure_installed` / highlight.enable.
-- Install parsers via the new API and start highlighting per filetype.
local ok, ts = pcall(require, 'nvim-treesitter')
if ok then
  ts.install({ "json", "yaml", "javascript", "typescript", "tsx", "go", "python", "terraform", "html", "css" })
  vim.api.nvim_create_autocmd('FileType', {
    pattern = { "json", "yaml", "javascript", "typescript", "typescriptreact", "go", "python", "terraform", "html", "css" },
    callback = function() pcall(vim.treesitter.start) end,
  })
else
  vim.notify('[nvim-treesitter] load failed: ' .. tostring(ts), vim.log.levels.ERROR)
end
EOF
